...
import (
	...
	"net/http"
	"time"
	"transaction/internal/db/entities"
...
type NullTransactionResponse struct {
...
func (ts *TransactionService) GetTransactionByID(w http.ResponseWriter, r *http.Request) {
	...
	if err := ts.DB.Where("id = ?", req.TransactionID).Find(&transaction).Error; err != nil || transaction.ID == "" {
		response := SOAPEnvelope{
			Body: SOAPBody{
				NullTransactionResponse: &NullTransactionResponse{
					Transactions: []string{""},
