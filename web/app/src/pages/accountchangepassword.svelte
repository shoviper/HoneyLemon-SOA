...
<Card class="w-full max-w-lg mx-auto">
  <form class="flex flex-col space-y-6" action="/">
    <div class="flex items-center justify-between">
      <h3 class="text-4xl font-bold text-lime-900 dark:text-white">Change Password</h3>
      <img src="{HoneyLemonLogo}" class="h-18 w-28" alt="HoneyLemonLogo" />
    </div>
    <Label class="space-y-2">
      <span class="text-gray-400">Enter the current password</span>
      <Input type="password" name="oldpassword" placeholder="Current password" required />
    </Label>
    <Label class="space-y-2">
      <span class="text-gray-400">Enter the new password</span>
      <Input type="password" name="newpassword" placeholder="New password" required />
    </Label>
    <Label class="space-y-2">
      <span class="text-gray-400">Confirm new password</span>
